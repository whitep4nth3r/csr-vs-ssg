      ...
      <nav className={HeaderStyles.header__nav} role="navigation">
        ...
        <ul className={navLinksClasses}>
          {Config.menuLinks.map((link) => {
            ...
            const onUsesPath = router.pathname.startsWith("/uses") && link.path.startsWith("/uses");

            const isActive =
              onBlogPost || onBlogIndexPage || onUsesPath || router.pathname === link.path;

            const isActiveClass = isActive ? ` ${HeaderStyles.header__navListItem__active}` : "";
